...
const switchOff = "bootstrap-switch wrapper bootstrap-switch-off bootstrap-switch-animate";
 class Setting extends Component {
     ...
     toggleSwitch(switchName){
        ...
        if(switchName == "switchSidebarMini")
        document.getElementsByTagName("body")[0].classList.toggle('sidebar-mini');
        if(switchName == "switchFixedNavbar")
        document.querySelector("nav.navbar.navbar-default").classList.toggle('navbar-fixed');
                ...
                <ul className="dropdown-menu">
                    ...
                    <li className="adjustments-line">
                        <a className="switch-trigger">
                            <p>Filters</p>
                            <div className="pull-right">
                                <Filter active={this.state.active} sidebarColor={this.props.color}  activeColor = {(value)=>this.setState({active:value})} className="badge filter" color="black" />
                                <Filter active={this.state.active} sidebarColor={this.props.color}  activeColor = {(value)=>this.setState({active:value})} className="badge filter badge-azure" color="azure" />
                                <Filter active={this.state.active} sidebarColor={this.props.color}  activeColor = {(value)=>this.setState({active:value})} className="badge filter badge-green" color="green" />
                                <Filter active={this.state.active} sidebarColor={this.props.color}  activeColor = {(value)=>this.setState({active:value})} className="badge filter badge-orange" color="orange" />
                                <Filter active={this.state.active} sidebarColor={this.props.color}  activeColor = {(value)=>this.setState({active:value})} className="badge filter badge-red" color="red" />
                                <Filter active={this.state.active} sidebarColor={this.props.color}  activeColor = {(value)=>this.setState({active:value})} className="badge filter badge-purple" color="purple" />
                            </div>
                            <div className="clearfix" />
                        </a>
                    </li>
